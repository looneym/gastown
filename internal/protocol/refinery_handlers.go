...
func (h *DefaultRefineryHandler) HandleMergeReady(payload *MergeReadyPayload) error {
	...
	// Determine target branch based on epic
	target := h.determineTargetBranch(payload.Issue)
	if target == "" {
		return fmt.Errorf("failed to determine target branch for issue %s", payload.Issue)
	}
...
// determineTargetBranch determines the target branch for a merge request.
// If the issue is part of an epic, returns "integration/<epic-id>".
// Otherwise returns "main" (which may be blocked by validation later).
func (h *DefaultRefineryHandler) determineTargetBranch(issueID string) string {
	if issueID == "" {
		return "main"
	}
	...
	if err != nil {
		_, _ = fmt.Fprintf(h.Output, "[Refinery] Warning: could not read issue %s: %v\n", issueID, err)
		return "main"
	}

	// If issue has an epic (parent), use integration/<epic-id>
	if issue.Parent != "" {
		return fmt.Sprintf("integration/%s", issue.Parent)
	}

	// No epic, use main (will be validated by engineer)
	return "main"
}
